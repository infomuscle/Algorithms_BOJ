...
deque = [x+1 for x in range(0,n)]
required = list(map(int, sys.stdin.readline().split()))

cnt = 0
for i in range(0, m):
    if deque[0] == required[i]:
        deque = popFront(deque)
    else:
        cntLeft, cntRight = 0,0
        tempLeft, tempRight = deque[0:], deque[0:]
        while (tempLeft[0] != required[i]):
            tempLeft = moveLeft(tempLeft)
            cntLeft += 1
        while (tempRight[0] != required[i]):
            tempRight = moveRight(tempRight)
            cntRight += 1
        if cntLeft < cntRight:
            cnt += cntLeft
            for j in range(0, cntLeft):
